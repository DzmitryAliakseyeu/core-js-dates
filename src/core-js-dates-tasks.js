...
function getDayName(date) {
  ...
  const dayInDate = new Date(date).getDay();
  return daysOfWeek[dayInDate];
}
...
function getNextFriday(date) {
  ...
  const minute = 60;
  const second = 60;
  const millisecondsInDay = hoursInDay * minute * second * 1000;
  const millisecondsDate = date.getTime();
  const dayOfDate = date.getDay();
  let dayUntilFriday = (5 - dayOfDate + 7) % 7;
  if (dayUntilFriday === 0) {
    dayUntilFriday = 7;
  }
  return new Date(millisecondsInDay * dayUntilFriday + millisecondsDate);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const dateStartMill = new Date(dateStart).getTime();
  const dateEndMill = new Date(dateEnd).getTime();
  const diff = dateEndMill - dateStartMill;
...
function isDateInPeriod(date, period) {
  const dateInMill = new Date(date).getTime();
  const startDayInMill = new Date(period.start).getTime();
  const endDayInMill = new Date(period.end).getTime();
  if (dateInMill >= startDayInMill && dateInMill <= endDayInMill) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  const dateInMill = new Date(date);
  const curDate = dateInMill.getUTCDate();
  const curMonth = dateInMill.getUTCMonth() + 1;
  const curYear = dateInMill.getUTCFullYear();
  let curHour = dateInMill.getUTCHours();
  const curMinutes = String(dateInMill.getUTCMinutes()).padStart(2, '0');
  const curSeconds = String(dateInMill.getUTCSeconds()).padStart(2, '0');
  const timePer = curHour >= 12 ? 'PM' : 'AM';
  curHour = curHour % 12 || 12;
  return `${curMonth}/${curDate}/${curYear}, ${curHour}:${curMinutes}:${curSeconds} ${timePer}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfYear = new Date(Date.UTC(copyDate.getUTCFullYear(), 0, 1));
  const millisecondsInDay = 1000 * 60 * 60 * 24;
  const weekNumber = Math.ceil(
    ((copyDate - firstDayOfYear) / millisecondsInDay + 1) / 7
  );
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  const copyDate = new Date(date.getFullYear(), date.getMonth(), 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const arrWorkingDays = [];
  let countWD = 0;
  let countOD = 0;
  let isWorkDay = true;

  const startDate = convert(period.start);
  const endDate = convert(period.end);

  while (startDate <= endDate) {
    const updateDate = `${String(startDate.getUTCDate()).padStart(2, '0')}-${String(startDate.getUTCMonth() + 1).padStart(2, '0')}-${startDate.getUTCFullYear()}`;

    if (isWorkDay) {
      arrWorkingDays.push(updateDate);
      countWD += 1;
      if (countWD === countWorkDays) {
        isWorkDay = false;
        countWD = 0;
      }
    } else {
      countOD += 1;
      if (countOD === countOffDays) {
        isWorkDay = true;
        countOD = 0;
      }
    }
